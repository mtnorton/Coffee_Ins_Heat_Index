...
# Update Sept 2016 - to calculate 2 indexes for two periods

# Gov't and CC data is in different format, so code is different
# Here: get data as a column starting 2/1 or 7/1 of each year, take sum
...
# Read data from CSV
######################
indexes_cc1 <- matrix(NA, 50, 26)
indexes_cc2 <- matrix(NA, 50, 26)
total_missing_cc1 <- matrix(NA, 50, 26)
total_missing_cc2 <- matrix(NA, 50, 26)
...
for (i in (1:nrow(rawdata_cc)))
{
  ...
  if (year < 2014)
  {
    ...
    { 
      ...
      # Count missing
      total_missing_cc1[sta_count, ind_year] <- length(which(is.na(rawdata_cc[i:(i+numdays),7])))
      
      if ((rawdata_cc[(i+numdays),1]==sta1) & (rawdata_cc[(i+numdays),2]==sta2) & (rawdata_cc[(i+numdays),3]==sta3))
      {indexes_cc1[sta_count, ind_year] <- mo_data}
    }
    
    if ((month==7) & (rawdata_cc[i,6]==1))
    { 
      ind_year <- year-1987
      numdays <- 215
      if (year%%4==3) {numdays <- numdays+1}
      ...
      # Count missing
      total_missing_cc2[sta_count, ind_year] <- length(which(is.na(rawdata_cc[i:(i+numdays),7])))
      
      if ((rawdata_cc[(i+numdays),1]==sta1) & (rawdata_cc[(i+numdays),2]==sta2) & (rawdata_cc[(i+numdays),3]==sta3))
      {indexes_cc2[sta_count, ind_year] <- mo_data}
    }
  }
...
indexes_cc1 <- indexes_cc1[,-26]
indexes_cc2 <- indexes_cc2[,-26]
total_missing_cc1 <- total_missing_cc1[,-26]
total_missing_cc2 <- total_missing_cc2[,-26]
# delete rows 1, 10, 14, 16, 20, 48
# Not in coordinates file
indexes_cc1 <- indexes_cc1[-c(1,10,14,16,20,48),]
indexes_cc2 <- indexes_cc2[-c(1,10,14,16,20,48),]
total_missing_cc1 <- total_missing_cc1[-c(1,10,14,16,20,48),]
total_missing_cc2 <- total_missing_cc2[-c(1,10,14,16,20,48),]

#sta_output_cc <- matrix(NA, 44, 3)

total_missing_perc_cc1<- total_missing_cc1/215
total_missing_perc_cc2<- total_missing_cc2/212
screened_indexes_cc1 <- indexes_cc1
screened_indexes_cc1[which(total_missing_perc_cc1>0.05)] <- NA
screened_indexes_cc2 <- indexes_cc2
screened_indexes_cc2[which(total_missing_perc_cc2>0.05)] <- NA
